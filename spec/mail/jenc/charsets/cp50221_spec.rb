RSpec.describe Mail::Jenc do
  context 'cp50221' do
    ...
    it 'encodes from address' do
      expect(mail.header[:from].value).to include(b_encode("ｻｼﾀﾞｼﾆﾝ", 'cp50221'))
    end

    it 'encodes to address' do
      expect(mail.header[:to].value).to include(b_encode("ｱﾃｻｷ", 'cp50221'))
    end

    it 'encodes subject' do
      expect(mail.header[:subject].value).to include(b_encode("①ｹﾝﾒｲ―∥￣－～", 'cp50221'))
    end

    it 'encodes body' do
      expect(mail.parts[0].body.raw_source.dup.force_encoding('cp50221')).to include(encode("ﾎﾝﾌﾞﾝ", 'cp50221'))
    end
    ...
    it 'encodes filename' do
      ...
      expect(field.parameters['filename']).to include(b_encode("①ﾃﾝﾌﾟﾌｧｲﾙ.txt", 'cp50221'))
    end
